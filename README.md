...
3. 新建用户脚本。
4. 将 `hydrachs 脚本功能 Patch.js` 的完整内容复制进去。
5. 保存并启用本 Patch 脚本。
6. 打开 [2026 年暑假研修课程](https://basic.smartedu.cn/training/dc6d78f2-bad8-4d09-b8da-0d758803dbe4) 页面。
7. 点击 hydrachs 控制面板里的 `准备刷课`。
8. 脚本会等待 4 秒，刷新页面并进入视频学习页。
9. 当前视频完成后，脚本会自动进入下一个白名单视频。
10. 当前课程全部完成后，脚本会跳转到下一个课程页面。
...
如果页面无法识别用户名，可以点击进度 Panel 中的 `账号` 按钮，手动输入账号标识。留空则恢复自动识别。
